// src/app/components/Hover3DCard.js
'use client';
import { motion, useMotionValue, useTransform } from 'framer-motion';
import { useRef } from 'react';

// --- Default Animation Variant (can be overridden) ---
const defaultItemVariants = {
  hidden: { opacity: 0, x: -50 }, // Slight horizontal entrance for inline list
  visible: { opacity: 1, x: 0, transition: { type: 'spring', stiffness: 100, damping: 15 } },
};

export default function Hover3DCard({
  IconComponent,
  name, // Name is now passed directly as a prop
  className = '',
  transitionDelay = 0,
  variants = defaultItemVariants,
  children, // Retained for flexibility, but usually name is used
}) {
  ...
  // Parallax transforms
  const rotateX = useTransform(y, [-100, 100], [10, -10]); // Reduced rotation for horizontal element
  ...
  return (
    ...
    className={`flex items-center gap-3 p-3 md:p-4 rounded-full bg-background/30 border border-accent/20 
  ...
  >
      ...
      {/* iOS internal border for glass effect (Adjusted to rounded-full) */}
      ...
      <div className="relative z-20 flex items-center gap-3 w-full">
        ...
        {/* ICON SLOT: Enhanced Icon Style (Higher Z-depth) */}
        {IconComponent && (
            <div 
              className="p-1.5 rounded-full bg-accent text-background shadow-lg" 
              style={{ transform: 'translateZ(15px)' }} // Pull icon forward in 3D space
            >
