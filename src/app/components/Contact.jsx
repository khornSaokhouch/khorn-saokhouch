...
const itemVariants = { hidden: { opacity: 0, y: 30 }, visible: { opacity: 1, y: 0, transition: { type: 'spring', stiffness: 90, damping: 15 } } };
...
export default function Contact() {
  ...
  return (
    <motion.section
      id="contact"
      className="min-h-screen flex flex-col items-center justify-center px-4 sm:px-8 md:px-20 lg:px-40 py-16 sm:py-24"
      initial="hidden"
      whileInView="visible"
      ...
      <div className="grid grid-cols-1 md:grid-cols-12 gap-8 w-full max-w-6xl">
        ...
        <motion.div className="md:col-span-7 w-full order-2 md:order-1" variants={itemVariants}>
          <motion.form variants={containerVariants} initial="hidden" whileInView="visible" viewport={{ once: true, amount: 0.2 }} className="w-full flex flex-col gap-4 sm:gap-5 bg-card/70 p-6 sm:p-8 rounded-3xl backdrop-blur-md border border-accent/10 shadow-2xl">
            <motion.input variants={itemVariants} type="text" placeholder="Your Name" className="p-3 sm:p-4 rounded-xl bg-background border border-gray-700 focus:border-accent text-white outline-none transition-colors shadow-inner shadow-black/20 text-sm sm:text-base" />
            <motion.input variants={itemVariants} type="email" placeholder="Your Email" className="p-3 sm:p-4 rounded-xl bg-background border border-gray-700 focus:border-accent text-white outline-none transition-colors shadow-inner shadow-black/20 text-sm sm:text-base" />
            <motion.textarea variants={itemVariants} rows="6" placeholder="Your Message" className="p-3 sm:p-4 rounded-xl bg-background border border-gray-700 focus:border-accent text-white outline-none transition-colors shadow-inner shadow-black/20 text-sm sm:text-base"></motion.textarea>
